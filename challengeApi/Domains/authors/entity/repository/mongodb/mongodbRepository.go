...
const CollectionName = model.DBCOLLECTION_AUTHORS

func (r *repo) Create(objectToCreate *model.Author) error {
...
func (r *repo) GetOne(queryFilter *model.QueryFilters) (objectToReturn model.Author, errFind error) {
	collection := setCollection(r.session, CollectionName)
...
func (r *repo) GetAll(filter model.QueryFilters) (objectsToReturn []model.Author, errFind error) {
...
func (r *repo) Update(filter *model.QueryFilters, objectToUpdate model.Author) error {
	collection := setCollection(r.session, CollectionName)
